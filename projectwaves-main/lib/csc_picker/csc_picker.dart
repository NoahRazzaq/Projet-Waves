...
class CSCPicker extends StatefulWidget {
  ...
  final String? defaultDeviceCountry;
...
class CSCPickerState extends State<CSCPicker> {
  ...
  void _setDefaultCountry({name = "France"}) {
    _onSelectedCountry(_country[CscCountry.values.byName(name).index+1]!);
  }

  ///Read JSON country data from assets
  Future<dynamic> getResponse() async {
    var res = await rootBundle
        .loadString('lib/csc_picker/assets/country.json');
  ...
  Future<List<String?>> getCountries() async {
    ...
    if (_countryFilter.isNotEmpty) {
      ...
      countries.forEach((data) {
        ...
        if(_currentLocale == data["iso2"]){
          _setDefaultCountry(name: data["name"]);
        }
      });
    }
    //_setDefaultCountry();
